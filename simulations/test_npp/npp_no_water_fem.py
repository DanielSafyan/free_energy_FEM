"""
FEM simulation for Nernst-Planck-Poisson equations.

This simulation models the time-dependent concentration of two ionic species
coupled with the electrostatic potential in an electrolyte.
"""
...
class NernstPlanckPoissonSimulation:
    ...
    def _assemble_residual_and_jacobian(self, c1, c2, phi, c1_prev, c2_prev):
        ...
        # # For testing: Verify that the poisson mechanism is correct
        # K_c1c1 = K_c1c1 * 0.0
        # K_c2c2 = K_c2c2 * 0.0
    ...
    def run(self, c1_initial, c2_initial, phi_initial, num_steps,rtol = 1e-2,atol = 1e-6, max_iter=30):
...
if __name__ == '__main__':
    ...
    sim = NernstPlanckPoissonSimulation(mesh, dt, D1, D2, z1, z2, epsilon, R, T, L_c, c0, alpha=1 , alpha_phi=1, chemical_potential_terms=chemical_potential_terms)
    ...
    phi_initial_dim = 0.1* spsolve(A_poisson, rhs_poisson)
